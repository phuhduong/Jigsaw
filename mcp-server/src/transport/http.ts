...
interface Session {
  ...
  server: DigiKeyServer;
}

const sessions = new Map<string, Session>();

function createStandaloneServer(clientId: string, clientSecret: string): DigiKeyServer {
  return new DigiKeyServer(clientId, clientSecret);
}
...
async function createNewSession(
  ...
  const transport = new StreamableHTTPServerTransport({
    ...
    onsessioninitialized: (sessionId) => {
      ...
      console.log('New DigiKey session created:', sessionId);
    },
  });
  ...
  transport.onclose = () => {
    if (transport.sessionId) {
      ...
      console.log('DigiKey session closed:', transport.sessionId);
    }
  };
...
function logServerStart(config: Config): void {
  ...
  console.log(`DigiKey MCP Server listening on ${displayUrl}`);
